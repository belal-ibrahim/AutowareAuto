...
class XSENS_DRIVER_PUBLIC XsensTranslator
{
public:
  class Config
  {
...
private:
  ...
  explicit XsensTranslator(const Config & config);

  bool convert(const Packet & pkt, std::vector<sensor_msgs::msg::Imu> & output);
  ...
  void parse_mtdata2(std::vector<sensor_msgs::msg::Imu> & output);
