...
public:
  ...
  inline uint64_t index(const PointT & pt) const
  {
  ...
  PointT centroid(const uint64_t index) const
  {
    ...
    PointT pt;
    common::geometry::point_adapter::set_point(pt,
      ((static_cast<float>(xdx) + 0.5F) * m_voxel_size.x) + m_min_point.x,
      ((static_cast<float>(ydx) + 0.5F) * m_voxel_size.y) + m_min_point.y,
      ((static_cast<float>(zdx) + 0.5F) * m_voxel_size.z) + m_min_point.z);
    return pt;
  }
